    ...
    if request.method == 'POST':
        ...
        if posting_form.is_valid() and len(images) <= 5:
            ...
            for image in images:
                ...
                image_form = ImageForm(files=request.FILES)  # Form 류는 request 로 시작하는 객체여야만 사용가능.
...
def update_posting(request, posting_id):
    ...
    if request.user == posting.author:
        if request.method == 'POST':
            form = PostingForm(reqeust.POST, instance=posting)
            if form.is_valid():
                posting = form.save()
                return redirect(posting)
        else:
...
def toggle_like(request, posting_id):
    ...
    # 좋아요를 누른 user 라면,
    if posting.like_users.filter(id=user.id).exist():
