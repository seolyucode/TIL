...
BFS(너비 우선 탐색) : 그래프 알고리즘으로, 목적은 임의의 한 정점에서 시작해서 모든 정점을 1번씩 방문하는 것

DFS의 목적도 임의의 한 정점에서 시작해서 모든 정점을 1번씩 방문하는 것이다.

같은 그래프 탐색 알고리즘으로 목적은 같으나 방문하는 방법이 다르다.

BFS는 현재 정점과 연결된 모든 정점 중 아직 방문하지 않은 것을 모두 방문. 방문 => 큐

BFS는 특별한 조건(모든 가중치가 1)에서 최단거리를 구할 수 있다.

DFS는 한 정점과 연결된 모든 간선 중 하나를 먼저 간 후 탐색하고 다시 또 하나를 가서..

성질이 다르다.

### 15653번 구슬 탈출 4 

최소 몇 번(기울이기) 만에 빨간 구슬을 구멍을 통해 빼낼 수 있는지

보드의 상태 - 빈칸, 벽, 구멍(<- 연산해도 변하지 않음) / 구슬(위치가 변함)

빨간 구슬 / 파란 구슬 위치가 문제의 정점

기울였을 때 빨간 구슬과 파란 구슬의 위치가 변함

정점이 다른 정점으로 이동 => 간선

최소 몇번(기울이기)이냐고 물었으니 가중치가 1인 간선으로 봐도 되고 BFS로 풀면 됨
